    ...
    ui(new Ui::MonitorWindow)
{
    ...
    MonitorWindow::QTextDispTab[ 0 ] = ui->textBrowser_Unit1;
    MonitorWindow::QTextDispTab[ 1 ] = ui->textBrowser_Unit2;
    MonitorWindow::QTextDispTab[ 2 ] = ui->textBrowser_Unit3;
    MonitorWindow::QTextDispTab[ 3 ] = ui->textBrowser_Unit4;
    MonitorWindow::QTextDispTab[ 4 ] = ui->textBrowser_Unit5;
    MonitorWindow::QTextDispTab[ 5 ] = ui->textBrowser_Unit6;
    MonitorWindow::QTextDispTab[ 6 ] = ui->textBrowser_Unit7;
    MonitorWindow::QTextDispTab[ 7 ] = ui->textBrowser_Unit8;
    MonitorWindow::QTextDispTab[ 8 ] = ui->textBrowser_Unit9;
    MonitorWindow::QTextDispTab[ 9 ] = ui->textBrowser_Unit10;
    MonitorWindow::QTextDispTab[ 10 ] = ui->textBrowser_Unit11;
    MonitorWindow::QTextDispTab[ 11 ] = ui->textBrowser_Unit12;
    MonitorWindow::QTextDispTab[ 12 ] = ui->textBrowser_Unit13;
    MonitorWindow::QTextDispTab[ 13 ] = ui->textBrowser_Unit14;
    MonitorWindow::QTextDispTab[ 14 ] = ui->textBrowser_Unit15;
    MonitorWindow::QTextDispTab[ 15 ] = ui->textBrowser_Unit16;
    ...
    MonitorWindow::onTimeout_DispSysTime();
 ...
 *
 ***********************************/
void MonitorWindow::onTcpClientButtonClicked()
{
    ...
    if (setupConnection(TCPCLIENT))
    {
        ...
        MonitorWindow::onTimeout_Monitor();
 ...
 *
 ***********************************/
void MonitorWindow::onTcpClientNewConnection(const QString &from, quint16 port)
{
 ...
 *
 ***********************************/
void MonitorWindow::onTcpClientStopButtonClicked()
{
    disconnect(ui->button_TcpClient, SIGNAL(clicked()), this, SLOT(onTcpClientStopButtonClicked()));
 ...
 *
 ***********************************/
void MonitorWindow::onTcpClientTimeOut()
{
 ...
 *
 ***********************************/
void MonitorWindow::onTcpClientDisconnectButtonClicked()
{
 ...
 *
 ***********************************/
void MonitorWindow::onTcpClientDisconnected()
{
...
//    ui->textBrowser_TcpClientMessage->setDisabled(true);
    ...
    for(quint16 idx = 0; idx < BAT_NUM; idx++ )
    {
        MonitorWindow::QTextDispTab[idx]->clear();
    }
    ui->lineEdit_MonitorTime->clear();
...
void MonitorWindow::disp_bat_info(void)
{
    QString detail = "";
    ...
    {
        detail = "";
        QTextCursor cursor(MonitorWindow::QTextDispTab[idx]->textCursor());
        ...
        if( bat_info_buff[idx].is_bat_conn != true )
        {
            ...
            {
                detail = detail + MonitorWindow::tr("电池连接状态： ") + "未连接" + "\n";
            }
            else
            {
                detail = detail + MonitorWindow::tr("电池连接状态： ") + "反接" + "\n";
            }

        }
        else
        {
            detail = detail + MonitorWindow::tr("电池连接状态： ") + "已连接" + "\n";
        }

        // 显示电池充电状态
        if( bat_info_buff[idx].is_bat_chgring != true )
        {
            detail = detail + MonitorWindow::tr("电池充电状态： ") + "未充电" + "\n";
        }
        else
        {
            detail = detail + MonitorWindow::tr("电池充电状态： ") + "充电中" + "\n";
        }

        // 显示bms连接状态，如果未连接，直接显示未连接
        if( bat_info_buff[idx].is_bms_conn != true )
        {
            detail = detail + MonitorWindow::tr("BMS连接状态： ") + "未连接" + "\n";

            // 直接显示到界面
            QColor color = MonitorWindow::QTextDispTab[idx]->textColor();
            MonitorWindow::QTextDispTab[idx]->setTextColor(Qt::gray);
            MonitorWindow::QTextDispTab[idx]->setText(detail);
            MonitorWindow::QTextDispTab[idx]->setTextColor(color);
            ...
            detail = detail + MonitorWindow::tr("BMS连接状态： ") + "已连接" + "\n";


        // 显示电池是否充满
        if( bat_info_buff[idx].is_bat_full == true )
        {
            detail = detail + MonitorWindow::tr("是否充满： ") + "已充满" + "\n";
        }
        else
            detail = detail + MonitorWindow::tr("是否充满： ") + "未充满" + "\n";

        detail = detail + MonitorWindow::tr("温度1： ") + QString::number( bat_info_buff[idx].tmp_1, 'f', 1 ) + MonitorWindow::tr("°") + "\n";
        detail = detail + MonitorWindow::tr("温度2： ") + QString::number( bat_info_buff[idx].tmp_2, 'f', 1 ) + MonitorWindow::tr("°") + "\n";
        detail = detail + MonitorWindow::tr("单体最高电压： ") + QString::number( bat_info_buff[idx].single_volt_max, 'f', 1 ) + MonitorWindow::tr(" V") + "\n";
        detail = detail + MonitorWindow::tr("单体最低电压： ") + QString::number( bat_info_buff[idx].single_volt_min, 'f', 1 ) + MonitorWindow::tr(" V") + "\n";
        detail = detail + MonitorWindow::tr("电池组电压： ") + QString::number( bat_info_buff[idx].bat_volt, 'f', 1 ) + MonitorWindow::tr(" V") + "\n";
        detail = detail + MonitorWindow::tr("电池组放电电流： ") + QString::number( bat_info_buff[idx].bat_discharge_current, 'f', 1 ) + MonitorWindow::tr(" A") + "\n";
        detail = detail + MonitorWindow::tr("电池组充电电流： ") + QString::number( bat_info_buff[idx].bat_chgr_current, 'f', 1 ) + MonitorWindow::tr(" A") + "\n";
        detail = detail + MonitorWindow::tr("剩余容量： ") + QString::number( bat_info_buff[idx].cap_remain, 'f', 1 ) + MonitorWindow::tr(" %") + "\n";
        detail = detail + MonitorWindow::tr("充电限流： ") + QString::number( bat_info_buff[idx].iout_limit, 'f', 1 ) + MonitorWindow::tr(" A") + "\n";
        detail = detail + MonitorWindow::tr("BMS告警字1： 0x") + QString::number( bat_info_buff[idx].warn_1, 16 ) + "\n";
        detail = detail + MonitorWindow::tr("BMS告警字2： 0x") + QString::number( bat_info_buff[idx].warn_2, 16 ) + "\n";
        detail = detail + MonitorWindow::tr("BMS告警字3： 0x") + QString::number( bat_info_buff[idx].warn_3, 16 ) + "\n";
        detail = detail + MonitorWindow::tr("BMS告警字4： 0x") + QString::number( bat_info_buff[idx].warn_4, 16 ) + "\n";
        detail = detail + MonitorWindow::tr("SOH值： ") + QString::number( bat_info_buff[idx].soh ) + MonitorWindow::tr("次") + "\n";
...
void MonitorWindow::onTcpClientAppendMessage(const QString &from, const QByteArray &message)
{
 ...
 *
 ***********************************/
void MonitorWindow::onTcpClientSendMessage()
{
#if 0
 ...
 *
 ***********************************/
bool MonitorWindow::setupConnection(quint8 type)
{
 ...
 *
 ***********************************/
void MonitorWindow::loadSettings()
{
...
void MonitorWindow::saveSettings()
{
    QSettings settings(settingsFileDir, QSettings::IniFormat);
...
// query timeout slot
void MonitorWindow::onTimeout_Query()
{
    qDebug("Query timeout");
}

// monitor timeout slot
void MonitorWindow::onTimeout_Monitor()
{
    ...
    QString detail = "";
    detail = detail + QString::number(m_monitor_time.hour, 10) + MonitorWindow::tr("时");
    detail = detail + QString::number(m_monitor_time.min, 10) + MonitorWindow::tr("分");
    detail = detail + QString::number(m_monitor_time.sec, 10) + MonitorWindow::tr("秒");
...
void MonitorWindow::onTimeout_DispSysTime()
{
